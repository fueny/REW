...
class Database:
    ...
    def add_history(self, file_id, card_index, user_id=None):
        """添加历史记录"""
        with self.get_connection() as conn:
            cursor = conn.cursor()
            ...
            if user_id:
                cursor.execute(
                    'SELECT id FROM history WHERE file_id = ? AND card_index = ? AND date = ? AND user_id = ? ORDER BY timestamp DESC LIMIT 1',
                    (file_id, card_index, today, user_id)
                )
            else:
                cursor.execute(
                    'SELECT id FROM history WHERE file_id = ? AND card_index = ? AND date = ? AND user_id IS NULL ORDER BY timestamp DESC LIMIT 1',
                    (file_id, card_index, today)
                )
            ...
            if existing:
                ...
                if user_id:
                    cursor.execute(
                        'INSERT INTO history (file_id, card_index, date, user_id) VALUES (?, ?, ?, ?)',
                        (file_id, card_index, today, user_id)
                    )
                else:
                    cursor.execute(
                        'INSERT INTO history (file_id, card_index, date) VALUES (?, ?, ?)',
                        (file_id, card_index, today)
                    )
    ...
    def get_history_by_date(self, file_id, date=None, user_id=None):
        ...
        with self.get_connection() as conn:
            ...
            if date is None:
                ...
                result = {}
                for d in dates:
                    # 获取该日期的所有卡片索引，按照卡片索引（文件中的顺序）排序
                    if user_id:
                        cursor.execute(
                            'SELECT DISTINCT card_index FROM history WHERE file_id = ? AND date = ? AND user_id = ? ORDER BY card_index',
                            (file_id, d, user_id)
                        )
                    else:
                        cursor.execute(
                            'SELECT DISTINCT card_index FROM history WHERE file_id = ? AND date = ? AND user_id IS NULL ORDER BY card_index',
                            (file_id, d)
                        )
